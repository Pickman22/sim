...
class Signal(Buffer):
    ...
    def _stream(self):
        ...
        if self.is_connected():
            ...
            logging.debug('Raw data: {}'.format(raw_data))
            self.lock.release()
        else:
...
class SocketSignal(Signal):
    ...
    def __init__(self, names, host, port, on_new_data=None, on_connect=None,
                 on_disconnect=None, buffer_size=1024):
        Signal.__init__(self, names, on_new_data, on_connect, on_disconnect)
    ...
    def _readline(self, *params):
        line = []
        while True:
            tmp = self.conn.recv(self._buffer_size)
            logging.debug('Socket data: {}'.format(tmp))
            if tmp == '':  # Connection dropped.
                self._is_connected = False
                return None
            else:
                return tmp
...
class SystemSimulation(Buffer):
    ...
    def _looper(self):
        ...
        self.t += self.rate
        self.add_dict_data(self.loop(self.t))
        if not self._stop:
